...
class CIFAR10DataProvider(DataProvider):
    ...
    def __init__(self, save_path=None, train_batch_size=256, test_batch_size=512, valid_size=None,
        ...
        if valid_size is not None:
            ...
            valid_dataset = datasets.CIFAR10(self.valid_path, train=False, transform=transforms.Compose([
                transforms.Resize(self.resize_value),
                transforms.CenterCrop(self.image_size),
                transforms.ToTensor(),
                self.normalize,
        ...
        self.test = torch.utils.data.DataLoader(
            datasets.CIFAR10(self.valid_path, train=False, transform=transforms.Compose([
                transforms.Resize(self.resize_value),
                transforms.CenterCrop(self.image_size),
                transforms.ToTensor(),
                self.normalize,
...
class TinyImagenetDataProvider(DataProvider):
    ...
    def save_path(self):
        if self._save_path is None:
            self._save_path = '/data/lifabing/tiny-imagenet-200'
        return self._save_path
