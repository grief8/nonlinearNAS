...
if __name__ == "__main__":
    ...
    if args.train_mode == 'search':
        ...
        topks = [1, 2, 6, 8, -1]
        for topk in topks:
            json.dump(trainer.export_top(topk_layer=topk, topk_block=-1), open(args.exported_arch_path.rstrip('.json') + '_layer{}_block-1.json'.format(topk), 'w'))
            json.dump(trainer.export_top(topk_layer=-1, topk_block=topk), open(args.exported_arch_path.rstrip('.json') + '_layer-1_block{}.json'.format(topk), 'w'))
